...
const DEMO_CULT_MULT := 2.0            # 修炼倍率：2.0≈挂一天通关
...
func _assign_mortal_job() -> Dictionary:
	...
	for j in MORTAL_JOBS:
		...
		weights.append(float(bias.get(j["id"], 1.0)))
	var total := 0.0
	for w in weights: total += w
	var roll := randf() * total
	var acc := 0.0
	var job: Dictionary = MORTAL_JOBS.back()
...
func _ready() -> void:
	...
	meditate_button.pressed.connect(_on_profile)
...
func _set_main_ui_visible(v: bool) -> void:
	if pet_group != null:
		pet_group.visible = v

	var vbox := get_node_or_null("PetGroup/VBox")
	if vbox != null:
		vbox.visible = v

	var buttons := get_node_or_null("PetGroup/VBox/Buttons")
	if buttons != null:
		buttons.visible = v

	if feed_button != null:
		feed_button.visible = v
	if meditate_button != null:
		meditate_button.visible = v
	if info_button != null:
		info_button.visible = v

	# language 已经搬到右键菜单，保持隐藏
	if language_button != null:
...
func _schedule_next_dialogue() -> void:
	...
	var t := get_tree().create_timer(wait)
	t.timeout.connect(func ():
		if not is_afk and not _is_cutscene_blocking():
			random_dialogue()
		_schedule_next_dialogue()
	)
...
func _reincarnate(next_perk: Dictionary = {}, next_personality: String = "", next_instinct: String = "") -> void:
	...
	var job = _assign_mortal_job()
	var old_luck := int(state.get("luck", 50))
	var stones_kept := int(int(state["spirit_stones"] / 2) * (1.0 + stone_keep_bonus))
	...
	state = {
		...
		"life_legendary_count": 0,
		"life_rare_count": 0,
		"breakthrough_success_total_lifetime": int(state.get("breakthrough_success_total_lifetime", 0)),
		"breakthrough_fail_total_lifetime": int(state.get("breakthrough_fail_total_lifetime", 0)),
		"pills_total_lifetime": int(state.get("pills_total_lifetime", 0))
	}
	# 注意：上面 state 被整体替换，state["mortal_job"] 重置为空字符串，
	# 但 _assign_mortal_job() 在替换之前已经写入了旧 state 的 mortal_job——
	# 这里需要重新把 job 结果写回新 state，否则刚分配的职业会被这次重置覆盖掉。
	state["mortal_job"] = job["id"]
...
func _drain_scheduled_chain_unlocks() -> void:
	...
	for entry in _scheduled_chain_unlocks:
		if int(entry.get("unlock_day", 0)) <= _day_counter:
...
func _open_shop() -> void:
	...
	_refresh_shop()
	shop_panel.size = Vector2.ZERO
	await get_tree().process_frame
	shop_panel.position = Vector2(-shop_panel.size.x / 2, SHOP_Y)
...
func _open_report_panel() -> void:
	...
	_refresh_reports()
	report_panel.size = Vector2.ZERO
	await get_tree().process_frame
	var win := get_viewport_rect().size
	report_panel.position = Vector2(win.x - report_panel.size.x - REPORT_MARGIN, REPORT_Y)
...
func _on_profile() -> void:
	...
	_refresh_profile()
	profile_panel.size = Vector2.ZERO
	await get_tree().process_frame
	var win := get_viewport_rect().size
	profile_panel.position = Vector2(win.x - profile_panel.size.x - REPORT_MARGIN, REPORT_Y)
...
func _exit_certificate_mode() -> void:
...
func _refresh_profile_current_life_tab(container: VBoxContainer) -> void:
	...
	# ── 下方：最近人生轨迹（横向时间线）──
	var trail_panel := PanelContainer.new()
	trail_panel.add_theme_stylebox_override("panel", _life_card_stylebox())
	var trail_margin := MarginContainer.new()
	for side in ["left", "right", "top", "bottom"]:
		trail_margin.add_theme_constant_override("margin_%s" % side, LIFE_CARD_PADDING)
	trail_panel.add_child(trail_margin)
	var trail_vb := _build_life_section_header("res://assets/ui/icon_scroll.png", "最近人生轨迹", "Recent Path")
	trail_margin.add_child(trail_vb)
	...
	var recent_milestones := _recent_life_milestones(5)
	if recent_milestones.is_empty():
		var none_label := Label.new()
		none_label.text = "暂无记录" if zh else "No records yet"
		none_label.add_theme_font_size_override("font_size", PROFILE_BODY_SIZE)
		none_label.add_theme_color_override("font_color", Color(0.5, 0.45, 0.35))
		timeline.add_child(none_label)
	else:
		for i in recent_milestones.size():
			var m: Dictionary = recent_milestones[i]
			var node_vb := VBoxContainer.new()
			node_vb.add_theme_constant_override("separation", 4)
			node_vb.alignment = BoxContainer.ALIGNMENT_CENTER

			var node_circle := PanelContainer.new()
			var circle_sb := StyleBoxFlat.new()
			circle_sb.bg_color = TRAIL_NODE_BG
			circle_sb.set_corner_radius_all(28)
			circle_sb.set_border_width_all(2)
			circle_sb.border_color = TRAIL_NODE_BORDER
			node_circle.add_theme_stylebox_override("panel", circle_sb)
			node_circle.custom_minimum_size = Vector2(56, 56)
			var node_icon := TextureRect.new()
			node_icon.custom_minimum_size = Vector2(28, 28)
			node_icon.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
			node_icon.set_anchors_preset(Control.PRESET_CENTER)
			var icon_path: String = _milestone_icon_path(m)
			if icon_path != "" and ResourceLoader.exists(icon_path):
				node_icon.texture = load(icon_path)
			node_circle.add_child(node_icon)
			node_vb.add_child(node_circle)

			var title_lbl := Label.new()
			title_lbl.text = String(m.get("title_zh","")) if zh else String(m.get("title_en",""))
			title_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
			title_lbl.add_theme_font_size_override("font_size", PROFILE_BODY_SIZE - 1)
			title_lbl.add_theme_color_override("font_color", LIFE_INK_COLOR)
			title_lbl.custom_minimum_size = Vector2(72, 0)
			title_lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
			node_vb.add_child(title_lbl)

			var desc_text: String = String(m.get("description_zh","")) if zh else String(m.get("description_en",""))
			if desc_text != "":
				var desc_lbl := Label.new()
				desc_lbl.text = desc_text
				desc_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
				desc_lbl.add_theme_font_size_override("font_size", PROFILE_BODY_SIZE - 2)
				desc_lbl.add_theme_color_override("font_color", LIFE_GOLD_COLOR)
				desc_lbl.custom_minimum_size = Vector2(72, 0)
				desc_lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
				node_vb.add_child(desc_lbl)

			timeline.add_child(node_vb)

			if i < recent_milestones.size() - 1:
				var connector := VBoxContainer.new()
				connector.alignment = BoxContainer.ALIGNMENT_CENTER
				connector.custom_minimum_size = Vector2(16, 56)
				var line := HSeparator.new()
				var line_sb := StyleBoxLine.new()
				line_sb.color = TRAIL_LINE_COLOR
				line_sb.thickness = 2
				line.add_theme_stylebox_override("separator", line_sb)
				connector.add_child(line)
				timeline.add_child(connector)

	trail_vb.add_child(timeline)
	container.add_child(trail_panel)
...
# ══════════════════════════════════════════
#  REINCARNATION CERTIFICATE (宽版布局)
# ══════════════════════════════════════════

const CERT_SIZE := Vector2(400, 270)
const CERT_TITLE_Y := 16.0
const CERT_BODY_TOP := 72.0
const CERT_BODY_BOTTOM := 220.0
const CERT_HINT_Y := 252.0     # "点击关闭"提示的纵向位置，确保落在卷轴底部装饰区域之外

var cert_bg: TextureRect
var cert_title_label: Label
var cert_body_vbox: VBoxContainer
...
func _build_certificate_panel() -> void:
	...
	# 一定要放在 Control.new() 后面
	certificate_panel.z_index = 300
	certificate_panel.top_level = true

	add_child(certificate_panel)

	# 背景：整张证书贴图，按比例显示，绝不拉伸/压扁
	cert_bg = TextureRect.new()
	cert_bg.texture = load("res://assets/ui/certificate_scroll.png")
	cert_bg.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	...
	# 标题：单独的Label，放进顶部牌匾区域，居中、字号大一点
	cert_title_label = Label.new()
	cert_title_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	cert_title_label.add_theme_font_size_override("font_size", 18)
	cert_title_label.add_theme_color_override("font_color", Color(0.35, 0.22, 0.1))
	cert_title_label.set_anchors_preset(Control.PRESET_TOP_WIDE)
	cert_title_label.position.y = CERT_TITLE_Y
	cert_title_label.size.x = CERT_SIZE.x
	certificate_panel.add_child(cert_title_label)

	# 正文：每个字段各自一个Label，放进VBox里，不再挤成一坨居中大段文字
	cert_body_vbox = VBoxContainer.new()
	cert_body_vbox.set_anchors_preset(Control.PRESET_TOP_WIDE)
	cert_body_vbox.position.x = 230.0
	cert_body_vbox.position.y = 76.0
	cert_body_vbox.size = Vector2(125.0, 130.0)
	cert_body_vbox.add_theme_constant_override("separation", 4)
	certificate_panel.add_child(cert_body_vbox)

	# 点击关闭提示：放在卷轴底部装饰区域之外，不压在红印上
	cert_hint_label = Label.new()
	cert_hint_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	cert_hint_label.add_theme_font_size_override("font_size", 10)
	cert_hint_label.add_theme_color_override("font_color", Color(0.5, 0.45, 0.35))
	cert_hint_label.set_anchors_preset(Control.PRESET_TOP_WIDE)
	cert_hint_label.position.y = CERT_HINT_Y
...
func _show_reincarnation_certificate(cause_zh: String, cause_en: String, life_num: int, highest_realm_idx: int, punchline: Dictionary = {}) -> void:
	...
	var title_text: String = title["zh"] if zh else title["en"]
	var cause_text: String = cause_zh if zh else cause_en

	cert_title_label.text = "轮回证书" if zh else "Reincarnation Certificate"

	for child in cert_body_vbox.get_children():
		child.queue_free()
	...
	if zh:
		...
		fields = [
			...
			"Cause:",
			cause_text,
			"Reincarnations: %d" % life_num,
		]

	# Punchline leads the certificate — comedy first, stats after.
	var punchline_text: String = String(punchline.get("zh","")) if zh else String(punchline.get("en",""))
	if punchline_text != "":
		fields.insert(0, punchline_text)
	...
	for line in fields:
		...
		l.add_theme_color_override("font_color", Color(0.3, 0.22, 0.12))
		cert_body_vbox.add_child(l)
